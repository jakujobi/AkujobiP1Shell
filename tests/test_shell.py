...
class TestBasicFunctionality:
    ...
    def test_empty_input_shows_prompt_again(
        self, mock_input_sequence, default_config, capsys
    ):
    ...
    def test_whitespace_only_shows_prompt_again(
        self, mock_input_sequence, default_config, capsys
    ):
    ...
    def test_shell_continues_after_command(
        self, mock_input_sequence, default_config, capsys
    ):
...
class TestBuiltinIntegration:
    ...
    def test_builtin_error_continues_shell(
        self, mock_input_sequence, default_config, capsys
    ):
        ...
        assert (
            "No such file or directory" in output or "cannot access" in output.lower()
        )

    def test_exit_with_arguments_ignores_args(
        self, mock_input_sequence, default_config, capsys
    ):
...
class TestExternalCommandIntegration:
    ...
    def test_external_command_executes(self, mock_input_sequence, default_config):
        ...
        with patch("builtins.input", mock_input_sequence("ls", "exit")):
            with patch(
                "akujobip1.shell.execute_external_command", return_value=0
            ) as mock_exec:
                run_shell(default_config)
    ...
    def test_command_not_found_continues_shell(
        self, mock_input_sequence, default_config, capsys
    ):
    ...
    def test_external_command_failure_continues_shell(
        self, mock_input_sequence, default_config
    ):
    ...
    def test_quoted_arguments_passed_correctly(
        self, mock_input_sequence, default_config
    ):
    ...
    def test_permission_denied_continues_shell(
        self, mock_input_sequence, default_config
    ):
    ...
    def test_signal_terminated_command_continues_shell(
        self, mock_input_sequence, default_config
    ):
        """Test that signal-terminated commands don't crash shell."""
        with patch("builtins.input", mock_input_sequence("some_cmd", "exit")):
            with patch(
                "akujobip1.shell.execute_external_command", return_value=130
            ):  # 128 + SIGINT
                exit_code = run_shell(default_config)
    ...
    def test_multiple_external_commands(self, mock_input_sequence, default_config):
        ...
        with patch(
            "builtins.input", mock_input_sequence("ls", "pwd", "echo test", "exit")
        ):
            with patch(
                "akujobip1.shell.execute_external_command", return_value=0
            ) as mock_exec:
...
class TestSignalHandling:
    ...
    def test_ctrl_c_during_input_continues(
        self, mock_input_sequence, default_config, capsys
    ):
    ...
    def test_ctrl_d_prints_exit_message(
        self, mock_input_sequence, custom_config, capsys
    ):
    ...
    def test_multiple_ctrl_c_continues(
        self, mock_input_sequence, default_config, capsys
    ):
        ...
        with patch(
            "builtins.input",
            mock_input_sequence(
                KeyboardInterrupt(), KeyboardInterrupt(), KeyboardInterrupt(), "exit"
            ),
        ):
            exit_code = run_shell(default_config)
    ...
    def test_ctrl_c_after_command_continues(
        self, mock_input_sequence, default_config, capsys
    ):
        """Test that Ctrl+C after command continues shell."""
        with patch(
            "builtins.input", mock_input_sequence("pwd", KeyboardInterrupt(), "exit")
        ):
            with patch("akujobip1.builtins.os.getcwd", return_value="/test"):
                exit_code = run_shell(default_config)
...
class TestErrorHandling:
    ...
    def test_parse_error_continues_shell(
        self, mock_input_sequence, default_config, capsys
    ):
        """Test that parse errors don't crash shell."""
        with patch(
            "builtins.input", mock_input_sequence('echo "unclosed quote', "exit")
        ):
            exit_code = run_shell(default_config)
    ...
    def test_unexpected_error_continues_shell(
        self, mock_input_sequence, default_config, capsys
    ):
        ...
        with patch("builtins.input", mock_input_sequence("test", "exit")):
            ...
            with patch(
                "akujobip1.shell.parse_command",
                side_effect=[RuntimeError("Test error"), ["exit"]],
            ):
                exit_code = run_shell(default_config)
...
class TestEdgeCases:
    ...
    def test_mixed_quotes(self, mock_input_sequence, default_config):
        ...
        with patch(
            "builtins.input", mock_input_sequence("""echo "hello" 'world' """, "exit")
        ):
            with patch(
                "akujobip1.shell.execute_external_command", return_value=0
            ) as mock_exec:
...
class TestConfigurationIntegration:
    ...
    def test_config_passed_to_parser(self, mock_input_sequence, default_config):
        ...
        with patch("builtins.input", mock_input_sequence("ls *.txt", "exit")):
            with patch(
                "akujobip1.shell.parse_command",
                side_effect=[["ls", "file.txt"], ["exit"]],
            ) as mock_parse:
                with patch(
                    "akujobip1.executor.execute_external_command", return_value=0
                ):
                    run_shell(default_config)
...
class TestBashTestSimulation:
    ...
    def test_bash_test_2_empty_then_exit(
        self, mock_input_sequence, default_config, capsys
    ):
        """
    ...
    def test_bash_test_3_unknown_command(
        self, mock_input_sequence, default_config, capsys
    ):
        """
        ...
        with patch("builtins.input", mock_input_sequence("defnotcmd", "exit")):
            with patch(
                "akujobip1.shell.execute_external_command", return_value=127
            ) as mock_exec:
                exit_code = run_shell(default_config)
    ...
    def test_bash_test_4_quoted_args(self, mock_input_sequence, default_config):
        ...
        """
        with patch(
            "builtins.input", mock_input_sequence('printf "%s %s\\n" "a b" c', "exit")
        ):
            with patch(
                "akujobip1.shell.execute_external_command", return_value=0
            ) as mock_exec:
    ...
    def test_all_bash_tests_sequence(self, mock_input_sequence, default_config, capsys):
        ...
        with patch(
            "builtins.input",
            mock_input_sequence(
                "", "defnotcmd", "echo test", "exit"  # empty  # unknown  # valid
            ),
        ):
            with patch("akujobip1.shell.execute_external_command", return_value=0):
                exit_code = run_shell(default_config)
